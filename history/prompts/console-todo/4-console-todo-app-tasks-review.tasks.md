---
id: 4
title: Console Todo App Tasks Review
stage: tasks
date: 2025-12-29
surface: agent
...
user: user
command: sp.tasks
labels: [tasks,console,todo,phase1,review]
links:
...
Review the `phase-1/tasks.md` file.
**Correction Needed:** You have included tasks for future phases (Web, Chatbot, K8s).
This file must ONLY contain tasks strictly related to **Phase I: In-Memory Python Console App**.

**Action:**
1. Remove all tasks related to FastAPI, Database, Next.js, Docker, or Kubernetes.
2. Keep only the tasks relevant to:
   - Project Setup (UV, Python)
   - Task Model (In-memory)
   - Task Manager Logic (CRUD)
   - CLI Interface (Menu)
   - Tests (pytest)
3. Rewrite `phase-1/tasks.md` with this corrected list.
...
- [ ] T001 Create project directory structure with src/, tests/, and pyproject.toml for console app
...
- [ ] T006 Create Status and Priority enum classes for console app
- [ ] T007 Create TodoManager class with in-memory storage (list/dict) for console app
...
- ✅ Impact: Reviewed and confirmed tasks are properly scoped to Phase I console app
- 🧪 Tests: All testing tasks are relevant to console app functionality
- 📁 Files: Updated tasks.md to emphasize console app focus
- 🔁 Next prompts: Ready for implementation of console app tasks
- 🧠 Reflection: Tasks correctly focused on in-memory console application requirements
